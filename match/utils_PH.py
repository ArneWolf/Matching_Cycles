# Author: Inés García-Redondo
# https://github.com/inesgare/interval-matching/blob/main/match/utils_PH.py
...
#from gudhi.representations.preprocessing import BirthPersistenceTransform # needed for prevalence-PI
...
def calculate_pds_dtm(X, k = 1, q = 2):
    simplicial_complex = DTMRipsComplex(X, None, k, q)
    simplex_tree = simplicial_complex.create_simplex_tree(max_dimension = 2) 
    pds = simplex_tree.persistence() 
...
# This is an adaptation of Gudhi's Persistence Image calculation:
# https://gudhi.inria.fr/python/latest/_modules/gudhi/representations/vector_methods.html#PersistenceImage.__init__
def compute_prevalence_persistence_images(X, prevalences, bandwidth = 0.01, im_range = [0, 2, 0, 2], resolution = [10, 10]):
    """
    Compute the persistence image for each persistence diagram individually, 
    and store the results in a single numpy array.
    ...
    for i in range(num_diag):
        ...
        x_values, y_values = np.linspace(im_range[0], im_range[1], resolution[0]), np.linspace(im_range[2], im_range[3], resolution[1])
        Xs, Ys = np.tile((diagram[:,0][:,np.newaxis,np.newaxis]-x_values[np.newaxis,np.newaxis,:]),[1,resolution[1],1]), np.tile(diagram[:,1][:,np.newaxis,np.newaxis]-y_values[np.newaxis,:,np.newaxis],[1,1,resolution[0]])
        
        
        image = np.tensordot(prevalences, 
                             np.exp((-np.square(Xs)-np.square(Ys))/(2*np.square(bandwidth)))/(np.square(bandwidth)*2*np.pi), 
                             1)
...
def compute_prevalence_persistence_image(pd, prevalences, bandwidth = 0.01, im_range = [0, 2, 0, 2], resolution = [10, 10]):
    pis =  compute_prevalence_persistence_images([pd], prevalences, bandwidth, im_range, resolution) 
    pi = pis[0]
    return pi


def sorted_lifespans_pd(PD, size = None):
    ...
    lifespans = np.around(lifespans, 2)
...
##### PARALLEL CALCULATION OF PREVALENCE



def calculate_prevalence_pd_in_parallel(X, list_Y):
    # X = point cloud
    # list_Y = [Y1, Y2, ..., Ym] list of resamples of X\
    
    num_resamples = len(list_Y)
    
    # Calculate PD(X).
    out_X = compute_bars_tightreps(X, filename = 'ldm_X')
    bars_X, _, _, _ = extract_bars_reps_indices(out_X, only_dim_1 = True) 
    pd = bars_X[1]
    num_bars = len(pd)
    
    print("Parallel calculation prevalence PD...")
    list_affinities = Parallel(n_jobs = 10)(delayed(calculate_affinities)(X, Y, num_bars, s) for s, Y in enumerate(list_Y))
    # print("list_affinities (parallel) = ", list_affinities)
    
    prevalences = np.zeros(num_bars)
    for i in range(num_bars):
        prevalences[i] = sum([list_affinities[s][i] for s in range(num_resamples)])
    prevalences = prevalences / num_resamples
    
    return pd, prevalences



# Auxiliary function to enable parallel computing of prevalence-augmented PD.
def calculate_affinities(X, Y, num_bars, s = 0):
    
    matched_X_Y, affinity_X_Y, bars_reps_X, bars_reps_Y = matching(X, Y, s, dim = 1, affinity_method = 'A', check_Morse = True)   
    affinities = np.zeros(num_bars)
...
def extract_bars_reps(out, only_dim_1 = False, verbose = False) :
    ...
    if not only_dim_1 :
        ...
        while i < line_PH[dim + 1] :
            ...
            if not x :
                x = re.search(r"\[(\d*.\d*),(\d*.\d* e-\d*)\)", out[i])
...
def shift_reps(rep, offset):
    return [[e + offset for e in edge] for edge in rep]
    ...
    if len(matched_X_Y) == 1 or not show_together :
        for match, aff in zip(matched_X_Y, affinity_X_Y) :
            ...
            if X.shape[1] == 2 :
                ...
                plot_cycreps(Y, [shift_reps(tight_reps_Y[dim][b], -len(X))], pts_to_show=Y, ax=axes[1])
                for ax in axes:
                    ax.set_aspect('equal')
                axes[0].set_xlabel('X')
                axes[1].set_xlabel('Y')
            ...
            if X.shape[1] == 3 :
                ...
                plot_cycreps(Y, [shift_reps(tight_reps_Y[dim][b], -len(X))], pts_to_show=Y, ax=ax)
                ax.set_title('Y')
                plt.show()
        ...
        if X.shape[1] == 2 :
            ...
            for match, aff in zip(matched_X_Y, affinity_X_Y) :
                ...
                plot_cycreps(Y, [shift_reps(tight_reps_Y[dim][b], -len(X))], pts_to_show=Y, ax=axes[i,1])
...
def matching(X,Y, s=0, dim = 1, verbose_figs = False, affinity_method = 'A', check_Morse = False) :
    ...
    ldm_file_X, ldm_file_Y, ldm_file_Z, threshold = \
        create_matrices_image(X, Y, filename_X = 'X_{}'.format(s), filename_Y = 'Y_{}'.format(s), filename_Z ='Z_{}'.format(s), return_thr = True)
...
####### Automate sampling strategies and prevalence calculations

def subsample_randomly(X, num_resamples, size_resamples):
    list_Y = []
    for i in range(num_resamples):
        ind = np.random.randint(0, X.shape[0], size=size_resamples)
        list_Y.append(X[ind,:])
    return list_Y

def compute_prevalence(X, list_Y):
    # X: original point cloud saved as np.array
    # list_Y: list of re-sampled spaces to match to X
    ...
    scores = np.zeros(num_PD1)
    for s in range(len(list_Y)) :
        for match, aff in zip(list_matched_X_Y[s], list_affinity_X_Y[s]):
            a, b = match
            scores[a] += aff
    scores /= num_resamples
    
    arg = argsort(PD1[:,0], option = 'asc')
    PD1_sorted = PD1[arg]
    scores_sorted = scores[arg]

    X=np.hstack((PD1_sorted, scores_sorted.reshape((PD1_sorted.shape[0],1))))
    return X


def matching_from_distmat(X,Y,filename_X="X", filename_Y="Y", filename_Z="Z", dim = 1, verbose_figs = False, affinity_method = 'A', check_Morse = False) :

    '''Function that takes as input two distance matrices X and Y and computes the relevant barcodes and the matching.'''
    ...
    maxi = max(np.max(X),np.max(Y))  # TODO: could also use different threshold for XZ, YZ
    ...
    for i in range(len(X)) :
        f.write(', '.join([ str(x) for x in X[i,:i]]))
        f.write('\n')
    f.close()

    f = open(ldm_file_Y, "w") # erase possibly pre-existing
    for i in range(len(Y)) :
        f.write(', '.join([ str(x) for x in Y[i,:i]]))
        f.write('\n')
    f.close()

    f = open(ldm_file_Z, "w") # erase possibly pre-existing
    for i in range(len(Z)) :
        f.write(', '.join([ str(x) for x in Z[i,:i]]))
        f.write('\n')
    f.close()
    
    threshold = maxi + 0.5 # to make sure we still include people <= maxi (in case maxi = 0... paranoia lol)
